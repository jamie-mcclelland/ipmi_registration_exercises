...
def dispColourOverlayImage(img1, img2, ax=None):
    """
  ...
  """
    ...
    # take transpose of images to put in matrix orientation as required by
    # imshow
    img1 = img1.T
    img2 = img2.T
    # scale images intensities between 0 and 1
    img1 = (img1 - np.nanmin(img1)) / (np.nanmax(img1) - np.nanmin(img1))
    img2 = (img2 - np.nanmin(img2)) / (np.nanmax(img2) - np.nanmin(img2))

    # create rgb image with red channel set to img1 and
    # green and blues channels set to img2
    ...
    # display image
    if not ax:
        ax = plt.imshow(img_rgb, origin='lower')
    else:
